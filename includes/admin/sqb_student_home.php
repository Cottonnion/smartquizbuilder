...
<div class="d-flex justify-content-between align-items-center mb-4" style="border-bottom: 1px solid #eee;">
    <h3 class="quiz--title m-0 border-bottom-0 "><i class="fa fa-cog" aria-hidden="true"></i> Student Dashboard</h3>
</div>

<?php 
global $wpdb;
$sqb_member_home = $wpdb->prefix .'sqb_member_home';
$results = $wpdb->get_results( "SELECT * FROM $sqb_member_home");
if(!empty($results)){
	?>
	<div class="member-MP--card mb-4 sqb_member_page_table_wrapper     position-relative   sqb-card-outer-gray " bis_skin_checked="1">
		...
		<a href="<?php echo admin_url('admin.php?page=sqb_create_student_page'); ?>" class="absoluteBtn btn create--btn mp_add_new_pages_btn mb-4 d-inline-block mp_type_engagement">Create New Student Dashboard Page</a>
					...
					<?php foreach ($results as $key => $row) {
						...
						<tr class="sqb_member_manage_page_row_id_<?php echo $id ?> odd" role="row">
							...
							<td align="left" class="engagement_td_shortcode">
								<div class="shortcode_table_td" bis_skin_checked="1"> <span id="eng_dynamic_copyable_text_<?php echo $id ?>">[SQBStudentDashboard id=<?php echo $id ?>][/SQBStudentDashboard]</span><span data-id="eng_dynamic_copyable_text_<?php echo $id ?>" id="copy_shortcode" class="copy-btn theme-button btn btn-info " onclick="sqb_member_copy_to_clipboard(this)"><i class="fa fa-files-o" aria-hidden="true"></i> Copy</span></div>
							</td>
							<td align="center" class="sorting_1"><?php echo $shortcode_date; ?></td>
							<td align="center"><a class="btn btn-info btn-sm" href="<?php echo admin_url('admin.php?page=sqb_create_student_page&id='.$id); ?>"><i class="fa fa-pencil"></i></a> <a class="btn btn-danger btn-sm delete_member_enagegement_by_id" data-id="<?php echo $id ?>" href=""><i class="fa fa-trash"></i></a></td>
						</tr>
					<?php } ?>
				</tbody>
			</table>
...
<div class="member-MP--card mb-4 sqb_member_page_table_wrapper     position-relative   sqb-card-outer-gray ">
		<div class="have-no-member-pages small-member-message" > 
			<div class="empty-no-pages-section" >
				...
				<h2>You don't have any Student Dashboard Page yet!</h2>
				<p>You can click the button to add a Student Dashboard Page.</p>
				<a href="<?php echo admin_url('admin.php?page=sqb_create_student_page'); ?>" class="manage-member-empty-btn"><i class="fa fa-plus-circle" aria-hidden="true"></i>Add your first Student Dashboard Page</a>
			</div>
		</div>
	</div>
</div>

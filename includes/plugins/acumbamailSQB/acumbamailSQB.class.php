...
class acumbamailSQB {
	...
	/* https://github.com/Acumbamail/Acumbamail-PHP-API
	* 
	* Get the token : First login to account after that visit this url https://acumbamail.com/en/apidoc/  
	*
	*/
    ...
    public function setAuthToken($auth_token) {
     ...
     public function register($email = '', $name = '', $params = '', $customFields = ''){
		...
		$merge_fields['tag'] = $tag;
		return $this->addSubscriber($list_id,$merge_fields);
     }
    ...
    **/
    ...
    public function getLists(){
        $request = "getLists";
        $lists_info =  $this->callAPI($request);

       
        
    
        $lists_data = null;
		if(is_array($lists_info) && !empty($lists_info)){
			$i = 0;
			foreach($lists_info as $list_id=>$list_info){
                $lists_data[$i]['list_id'] = $list_id;

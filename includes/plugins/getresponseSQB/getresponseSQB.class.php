...
class getresponseSQB  
{
    ...
    function register($email, $name, $params ,$customFields = ''){
		...
		try {
			...
			$send_params["campaign"] =  array("campaignId" => $listId);

			//$api_key = $params[1]; 
			$tag = '';
			$create_tag_id = null;
			$created_tag_ids_arr = array(); 
			if(isset($params[3]) && $params[3] != ''){
				...
				if(is_array($tags_arr) && count($tags_arr)){
					foreach($tags_arr as $tag){
						$create_tag_id = '';
						$create_tag_name = $tag;
						$create_tag_name = str_replace(' ','_',trim($create_tag_name));
						$create_tag_name = str_replace('-','_',trim($create_tag_name));
						
						$tabs_info_obj = $this->getTags();
						$create_new_tag = false;
						if(is_object($tabs_info_obj) && !empty($tabs_info_obj)){
							$create_new_tag = true;
							foreach($tabs_info_obj as $tab_info_obj){
								$tagId = $tab_info_obj->tagId;
								$name = $tab_info_obj->name;
								if($create_tag_name == $name){
									$create_tag_id = $tagId;
									$create_new_tag = false;
									break;
								}
						...
						if($create_new_tag){
							// creating tag code
							$tag_output = $this->createTags(array('name'=>$create_tag_name));
							$create_tag_id  = $tag_output->tagId;
							$created_tag_ids_arr[]['tagId'] = $create_tag_id; 
						}else{
							$created_tag_ids_arr[]['tagId'] = $create_tag_id;
						}
					}
			...
			if(count($created_tag_ids_arr)){
				$send_params["tags"] =  $created_tag_ids_arr;
			}
			

			$accounts_info = $this->addContact($send_params);
